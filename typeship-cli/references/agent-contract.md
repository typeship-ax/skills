...
- MCP: `typeship mcp install --all` registers https://typeship.dev/mcp with every agent client found (auth env var as a reference). Cursor is included when detected; use `--cursor` to configure it explicitly.
