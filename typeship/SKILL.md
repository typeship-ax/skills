...
typeship generates packages from an OpenAPI or GraphQL spec, which it calls a Definition. Each package type is a Target: CLI, Go CLI, MCP server, TypeScript SDK, Python SDK, or Go SDK. A Project links a Definition to its Targets and keeps them current by opening pull requests in each Target's repository.
...
1. **Generate a package now, without an account.** Run `typeship generate run --definition '{"url":"<spec-url>"}' --target '{"generator":"cli"}' --out <dir>`. Anonymous runs generate the first 25 operations. If the result includes `claim.url`, give that link to the user: once signed in, they can save the run as a Project with the same spec, Target, and config. Continue with `typeship-cli`.
2. **Work that needs an account.** Run `typeship auth check --format json` and branch on `status`. An existing `TYPESHIP_TOKEN` or stored login is enough. If a credential is needed, run `typeship login --no-browser`, give the approval link to the user, and wait. Never ask for a key in chat. Continue with `typeship-cli`.
3. **Keep generated packages current.** Continue with `typeship-cli` to find or create the Project, connect each Target to a repository, and turn on automatic generation.
...
- Errors arrive as one JSON envelope on stderr: `{status, issues: [{code, message}], next_steps}`. Branch on `issues[].code`, not the message.
  ...
  - `forbidden`: the sign-in belongs to another organization or the member's role does not allow the action. Requesting another scope will not help.
- Change the generated API through the Definition, a Definition patch, or Project and Target config, not by editing generated files. For a Target delivered to a repository, commit hand-written helpers, tests, and build changes to its Draft pull request. typeship carries them forward on every generation. Packages from `generate run` are not tracked, so later runs will not preserve edits.
